...
protected static LogManager logger = LogManager.getLoggerInstance(ProcessData.class.getSimpleName());
...
public enum Types_Of_Data {
   ...
   private void buildOutputAndErrorStreamData() throws IOException {
       ...
       try {
           ...
           logger.logInfo("Started retrieving data from streams of attached process: " + this.checked_process);
           ...
           while (isRunning(this.checked_process)||outputProduced) {   //Continue if process is alive or some output was produced on previous iteration and there may be still some data to read.
               ...
               sleep(100,false);                                  //Some local commands can exit fast, but immediate stream reading will give no output and after iteration, 'while' condition will be false so we will not read out any output while it is still there, just need to wait for some time for it to appear in streams.
...
logger.logError("!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@Unconditional exit occured@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@!\nsome process hag up for more than "+unconditionalExitDelayMinutes+" minutes.");
                       ...
                       logger.logError(exceptionAsString);
   ...
   public static void sleep(long millis, boolean logOutput) {
       if (logOutput) {
          logger.logInfo("Starting sleeping for " + (millis / 1000) + " seconds...");
          logger.logInfo("Caller: " + Thread.currentThread().getStackTrace()[2]);
       }
       try {
           Thread.sleep(millis);
